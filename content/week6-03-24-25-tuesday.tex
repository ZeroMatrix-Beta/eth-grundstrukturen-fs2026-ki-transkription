...
Ein Dedekindscher Schnitt muss nach unten abgeschlossen sein (Bedingung \textbf{(D1)}). Würden wir nur $p^2 < 2$ fordern, so läge etwa $1$ in der Menge ($1^2 = 1 < 2$), nicht aber die kleinere Zahl $-3$ (denn $(-3)^2 = 9 > 2$) --- \textbf{(D1)} wäre also verletzt. Daher nehmen wir alle negativen rationalen Zahlen explizit mit auf.
\end{explanation-of-steps}
\end{math-stroke}
...
\begin{itemize}
    ...
    \item Ein Dedekindscher Schnitt darf jedoch nach Axiom \textbf{(D2)} \textbf{kein maximales Element} besitzen!
\end{itemize}
...
\begin{enumerate}[label=\textbf{(\roman*)}]
    ...
    \item $\alpha < \beta :\iff \alpha \subsetneq \beta$
\end{enumerate}
\end{definition}
...
\begin{explanation-of-steps}
Im Skript wird die Multiplikation oft nur für positive reelle Zahlen explizit hingeschrieben, da die Fallunterscheidung für negative Zahlen rein technisch ist und keine neuen analytischen Einsichten liefert. Wichtig ist, dass bei der Multiplikation positiver Schnitte alle rationalen Zahlen $p < 0$ explizit mit aufgenommen werden, um die Bedingung \textbf{(D1)} (Abgeschlossenheit nach unten) zu erfüllen. \inlinemetanote{[Anmerkung: Der Fall für zwei positive Schnitte ($\alpha > 0_{\mathbb{R}} \land \beta > 0_{\mathbb{R}}$) ist in Fall 2 nicht separat aufgeführt, da er bereits vollständig durch Fall 1 abgedeckt ist.]}
\end{explanation-of-steps}
\end{math-stroke}

\begin{ai-note}[{In Fall 1 muss $p \le 0$ statt $p < 0$ stehen --- Opus 5}]
% [PERSISTENT]
So wie \cref{def:multiplikation-reell} oben notiert ist, fehlt der Menge $\alpha \cdot \beta$ genau das Element $0$: Die Bedingung $p < 0$ schliesst $0$ aus, und $0 = q \cdot r$ ist mit $q, r > 0$ unmöglich. Da $\alpha, \beta > 0_{\mathbb{R}}$ sind, gibt es aber nach \textbf{(D2)} rationale $q \in \alpha$, $r \in \beta$ mit $q, r > 0$, also liegt $q \cdot r > 0$ in $\alpha \cdot \beta$ --- und $0 < q \cdot r$, ohne dass $0$ selbst in der Menge wäre. Das verletzt \textbf{(D1)}. Korrekt lautet die erste Zeile daher
\[ \alpha \cdot \beta := \{ p \in \mathbb{Q} \mid p \le 0 \ \text{ oder } \ \exists q \in \alpha, r \in \beta \text{ mit } q, r > 0 \text{ und } p = q \cdot r \}. \]
Mit $p \le 0$ sind alle drei Bedingungen erfüllt: \textbf{(D1)} für $0 < y < q \cdot r$ folgt aus $y/r < q$ und der Abgeschlossenheit von $\alpha$; \textbf{(D2)} folgt für $p \le 0$ aus $q \cdot r > 0$ und für $p = q \cdot r$ aus einem $q' \in \alpha$ mit $q' > q$. Die anschliessende Erläuterung (\qt{alle rationalen Zahlen $p < 0$}) ist entsprechend als $p \le 0$ zu lesen.
\end{ai-note}
...
Verifizieren Sie die Axiome $\mathbf{R_0}$ bis $\mathbf{R_{16}}$ (Körper-, Ordnungs- und Kompatibilitätsaxiome) für das Modell der Dedekindschen Schnitte. Zeigen Sie insbesondere die Gültigkeit des Distributivgesetzes:
