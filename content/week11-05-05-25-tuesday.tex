...
Für jedes $k \ge 1$ existiert eine zyklische Folge $(a_1, \dots, a_{2^k})$ aller $2^k$ verschiedenen binären Wörter der Länge $k$, sodass sich aufeinanderfolgende Wörter $a_{\ell}$ und $a_{\ell+1}$ (sowie das letzte Wort $a_{2^k}$ und das erste Wort $a_1$) in genau einer Stelle unterscheiden. Ein solcher Zyklus entspricht für $k \ge 2$ genau einem Hamilton-Kreis im Hyperwürfel $Q_k$ \inlinemetanote{[Anmerkung: Für $k = 1$ existiert die Folge $(0,1)$ zwar, aber $Q_1$ ist nur eine einzelne Kante und enthält gar keinen Kreis --- der zyklische Rücklauf benutzt dieselbe Kante ein zweites Mal. Die Aussage \qt{$Q_k$ ist ein Hamilton-Graph} gilt daher, wie in der Vorlesung gesagt, erst ab $k \ge 2$.]}.
\end{proposition}
\end{math-stroke}
...
In einem bipartiten Graphen $G = (U, W, E)$ können wir allen Knoten in $U$ die Farbe Rot und allen Knoten in $W$ die Farbe Blau zuweisen. Da per Definition alle Kanten ausschließlich zwischen $U$ und $W$ verlaufen, gibt es keine Kanten zwischen gleichfarbigen Knoten. Umgekehrt liefert jede zulässige Zwei-Färbung $c$ unmittelbar eine Bipartition, indem man $U := c^{-1}(\text{Rot})$ und $W := c^{-1}(\text{Blau})$ setzt: Diese beiden Mengen sind disjunkt, überdecken $V$, und keine Kante verläuft innerhalb einer von beiden. Zusammen ergibt das die behauptete Äquivalenz.
\end{explanation-of-steps}
\end{math-stroke}
...
\begin{enumerate}[label=\textbf{(\arabic*)}]
    ...
    \]
    sowie die eingeschränkte Kantenmenge $E' := \bigl\{ \{a, b\} \in E \;\big|\; a \in A', \, b \in B' \bigr\}$. Der reduzierte Graph ist $G' = (A', B', E')$ mit $|A'| = n$.
