...
\section{Das Unendlichkeitsaxiom und die Menge \texorpdfstring{$\omega$}{omega} (Axiom 4)}
...
Die leere Menge $\emptyset$ ist \emph{nicht} induktiv, da sie die Bedingung $\emptyset \in \emptyset$ verletzt: $\emptyset$ besitzt überhaupt kein Element. \inlinemetanote{[Anmerkung: Dass \emph{keine} Menge sich selbst enthält, ist eine stärkere Aussage und folgt erst aus dem Fundierungsaxiom (\cref{ax:foundation}). Für $\emptyset$ braucht man dieses Axiom nicht.]}
\end{example}
\end{math-stroke}
...
\section{Das Aussonderungsaxiom und Klassenbildung (Axiom 5)}
...
\begin{explanation-of-steps}[Weshalb der Umweg über $\bigcup x$? Und was ist $\bigcap \emptyset$?]
Die Bedingung $\forall z \, (z \in x \rightarrow u \in z)$ allein beschreibt nur eine Kollektion, keine Menge --- das Aussonderungsaxiom (\cref{ax:specification}) verlangt eine bereits existierende Obermenge, aus der ausgesondert wird. Für $x \neq \emptyset$ ist $\bigcup x$ eine solche Obermenge, denn jedes $u$, das in \emph{allen} $z \in x$ liegt, liegt insbesondere in einem davon, also in $\bigcup x$.

Damit beantwortet sich auch die Frage aus dem Publikum nach $\bigcap \emptyset$: Da $\bigcup \emptyset = \emptyset$ ist, liefert die obige Definition $\bigcap \emptyset = \emptyset$. Naiv gelesen müsste $\bigcap \emptyset$ dagegen die Kollektion \emph{aller} Mengen sein (die Bedingung $z \in \emptyset \rightarrow u \in z$ ist für jedes $u$ leer erfüllt) --- und die ist nach \cref{ex:russell-paradox} keine Menge. Genau deshalb schränkt man $\bigcap$ üblicherweise auf nichtleere $x$ ein.
\end{explanation-of-steps}
\end{math-stroke}
...
\section{Das Potenzmengenaxiom (Axiom 6)}
...
\begin{math-stroke}
\begin{axioms}[Zermelo-Fraenkel-Mengenlehre (Teil 6)]
\begin{enumerate}[label=\textbf{(\arabic*)}]
    \setcounter{enumi}{6}
...
\begin{math-stroke}
\begin{axioms}[Zermelo-Fraenkel-Mengenlehre (Teil 7)]
\begin{enumerate}[label=\textbf{(\arabic*)}]
    \setcounter{enumi}{7}
