...
\begin{enumerate}[label=\textbf{(\roman*)}]
    ...
    \item $v$ ist injektiv.
    \item $v$ ist surjektiv. \inlinemetanote{[Anmerkung: Wäre $v$ nicht surjektiv, gäbe es ein Element $x = F(A \setminus \operatorname{bild}(v))$. Man könnte dann $v$ auf $\alpha \cup \{\alpha\}$ erweitern, indem man $\alpha$ auf $x$ abbildet. Diese Erweiterung wäre wieder eine $F$-Menge, was der Maximalität der Vereinigung $v$ widerspricht.]}
\end{enumerate}
\end{math-stroke}
