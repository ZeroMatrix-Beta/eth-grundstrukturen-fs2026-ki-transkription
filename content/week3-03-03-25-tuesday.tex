...
\begin{itemize}
    \item Die rein logischen Axiom-Anwendungen (wie Instanzen von $\mathbf{L_0}$--$\mathbf{L_{16}}$) meist unterdrückt werden.
...
Die dritte Zeile der obigen Gleichungskette ist textidentisch mit der zweiten (kein tatsächlicher Umformungsschritt), und die Endbegründung \qt{nach $\mathbf{PA_3}$ und $\mathbf{PA_2}$} überspringt die eigentliche Umgruppierung. Vollständig: $(x+x)+S(S(0)) = (x+S(S(0)))+x$ (Kommutativität/Assoziativität) $= S(S(x))+x$ (zweimal $\mathbf{PA_3}$ und $\mathbf{PA_2}$) $= S(x)+S(x)$ (Kommutativität und $\mathbf{PA_3}$). Das Endresultat bleibt korrekt.
\end{ai-note}
