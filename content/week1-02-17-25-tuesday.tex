...
\begin{enumerate}[label=\textbf{(VT\arabic*)}, start=0]
    \item Ist $\tau \equiv x$ eine Variable (eigentlich $\nu$, nicht $x$), so ist $\operatorname{var}(x) := \{x\}$.
